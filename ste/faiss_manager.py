...
class FAISSManager:
    ...
    def _rebuild_index_from_vectors(self) -> None:
        self.index = faiss.IndexFlatL2(self.dimension)
        if self.vectors.size:
            self.index.add(self.vectors.astype(np.float32, copy=False))
    ...
    def add(self, memory_id: str, vector: np.ndarray, embeddable_text: str | None = None) -> bool:
        ...
        if memory_id in self.memory_id_to_offset:
            offset = self.memory_id_to_offset[memory_id]
            self.vectors[offset] = arr[0]
            self._rebuild_index_from_vectors()
    ...
    def load_index(self, path: str | Path | None = None) -> None:
        ...
        if self.index.ntotal != len(self.memory_ids):
            raise ValueError(
                f"FAISS index/vector metadata mismatch: index has {self.index.ntotal} vectors, "
                f"metadata has {len(self.memory_ids)} memory_ids"
            )
        self._rebuild_mappings()
        self._refresh_vector_cache()
        self.index_path = target
